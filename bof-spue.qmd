...
## Compute sightings per unit effort for the Bay of Fundy

Utilize only R/V Nereid surveys.

Choose the designated conservation area, or a larger polygon
...
curr_dir <- '/Users/dan/Documents/WorkDocuments/Projects/BOF-SPUE'
setwd(curr_dir)
```
...
Define 'seasons'. Calculations of SPUE will be done within these time blocks, within each year
...
If data file is not found locally in /data/, download the data from your googledrive
...
Restrict data use only to the R/V Nereid
...
dat <- dat %>% 
  ...
  filter(fileid == "P" | fileid == "p") %>% dplyr::select(-fileid)
```

Convert timezones: GMT to ET and add columns for datetime and jday in ET
...
GMT_strings[which(GMT_strings == "02e+05")] = "200000" #correct instances where "200000" was stored as "02e+05"
...
Create Year, Month, and Day columns based on US/Eastern tz, discard data outside of range specified above
...
Create seasons matrix and columns with intra- and inter- annual seasons columns
...
Identify records that are ON vs OFF effort, per the conditions below. 0 = off effort. 1 = on effort.
...
Create unique identifier for continuous segments of on-effort Use cumsum(abs(diff())) to create unique, monotonically increasing, numbers to identify continuous segments of on- and off-effort
...
Select a subset of columns and discard the rest.
...
Add geomotery to the dataset, rename to `tmpdat` and convert to sf object
...
Define polygon within which we want SPUE
...
Generate one tracks file for each FILEID (survey). Cast as MULTILINESTRING. Intersect with the polygon. Determine which FILEIDs intersected the polygon, and discard those that do not intersect the polygon.

```{r}
# create vessel tracks for each FILEID
tracks <- tmpdat_sf %>% 
  group_by(FILEID, on.effort.id) %>% # group data by FILEID, and on/off effort sections within each FILEID
  arrange(FILEID, EVENTNO) %>% # make sure data is in order (should already be)
...
Plot all surveys that intersect the polygon.
...
Set up arrays to hold processed data.
...
## CREATE ARRAYS TO HOLD SPECIES DETECTION HISTORIES & DETECTION COVARIATE ARRAYS (POTENTIALLY FOR JAGS MODELLING)
...
for (i in 1:num_ssn){
  ...
  for (j in 1:num_season_ufids){
    ...
    intersection <- st_intersection(polygon_sf, nereid_tracks) %>%
      ...
      mutate(total_length_km = as.numeric(total_length)*0.001) %>% #changes length from [m] to <dbl> and converts from meters to km
      group_by(grid_id)
...
```
Copmpose SPUE object and plot SPUE vs Year
```{r}
